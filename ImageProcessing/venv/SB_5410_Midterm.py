...
class Application(Frame):
    ...
    def putImage(self, File):
        ...
        self.canvas = Canvas(self.upper, bd=0, width=self.photo.width(), height=self.photo.height(),
                             xscrollcommand=self.xscroll.set, yscrollcommand=self.yscroll.set)
        self.canvas.grid(row=0, column=0, columnspan=6, sticky=N + S + E + W)
    ...
    def create_widgets(self):
        ...
        # set up the main upper frame
        self.upper = Frame(root, bd=2, relief=SUNKEN)
        self.upper.grid_rowconfigure(0, weight=1)
        self.upper.grid_columnconfigure(0, weight=1)
        self.xscroll = Scrollbar(self.upper, orient=HORIZONTAL)
        self.xscroll.grid(row=1, column=0, sticky=E + W)
        self.yscroll = Scrollbar(self.upper)
        self.yscroll.grid(row=0, column=1, columnspan=6, sticky=N + S)
        # self.canvas = Canvas(self.upper, bd=0, xscrollcommand=self.xscroll.set, yscrollcommand=self.yscroll.set)
        # self.canvas.grid(row=0, column=0, sticky=N + S + E + W)
        # self.xscroll.config(command=self.canvas.xview)
        # self.yscroll.config(command=self.canvas.yview)
        self.upper.grid(row=0, column=0, columnspan=6, sticky=W)

        self.lower = Frame(root, bd=2, relief=SUNKEN)
        self.lower.grid_rowconfigure(1, weight=1)
        self.lower.grid_columnconfigure(1, weight=1)

        # # create resize check button
        self.is_resize = BooleanVar()
        Checkbutton(self.upper,
                    text="Resize",
        ...
        self.angle_ent = Entry(self.upper, width=10)
        ...
        self.is_horizontal = IntVar()
        self.is_radio = IntVar()
        self.rdioVer = Radiobutton(self.upper, text='Vertical',
                                   variable=self.is_radio, value=1)
        self.rdioHor = Radiobutton(self.upper, text='Horizontal',
                                   variable=self.is_radio, value=2)

        self.rdioVer.grid(column=0, row=4, sticky=E)
        self.rdioHor.grid(column=0, row=4, sticky=W)
        ...
        self.tolerance_ent = Entry(self.upper, width=8)
        ...
        self.generate_btn = Button(self.upper,
                                   text="Generate",
